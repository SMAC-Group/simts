...
The code below shows how to setup a time series oriented object with the `gts()` function. Here we take samples from each dataset at a rate of `freq` ticks per sample.
...
By applying the `plot()` on the result of a `gts()` simulation, we can observe a visualization of the data simulated. 
