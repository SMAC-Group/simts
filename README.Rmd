...
For users who are interested in having the latest and greatest developments in wavelets or simts methodology, this option is ideal. Though, there is considerably more work that a user must do in order to have a stable version of the package. **The setup to obtain the development version is platform dependent.** Most importantly, users **must** have a compiler installed on their machine that is compatible with R. (Clang, for example).
...
The following R packages are also required. If you have made it this far, run the following code in an R session and you will be ready to use the `simts` package.
